...
for (const elem in obj) {
    if (elem == 'lang') {
        var tplangs = obj[elem];
        for (const lang in tplangs) {
            var buffer = {}
            console.log('processing language', lang)
            const dir = __dirname + '/../easymodes/localization/' + lang;
            mkdirp.sync(dir);
            for (const trans in tplangs[lang]) {
                var file = dir + '/' + trans + '.json';
                buffer[trans] = tplangs[lang][trans]
                fs.writeFileSync(file, JSON.stringify(tplangs[lang][trans], null, '  '));
            }
            fs.writeFileSync(dir + '/loc.json', JSON.stringify(buffer, null, '  '));
        }
    } else {
        mkdirp.sync(__dirname + '/../easymodes/' + elem);

        const dev = obj[elem];
        for (const partner in dev) {
            var file = __dirname + '/../easymodes/' + elem + '/' + partner + '.json';
            if (dev[partner]) {
    ...
    lines.forEach(line => {
        if (match = line.match(/set PROFILE_([0-9]+)\(([A-Z_]+)\)\s+"?([^"]+)"?/)) {
            ...
            if (val = match[3].match(/\{([^}]*)}/)) {
                ...
                val = { readonly: true, val: match[3].trim() };
            }
            ...
            const subset = 'SUBSET_' + match[1];
            var param = match[2];
            var val = match[3];
            if (globalSubsets[subset] === undefined) {
                globalSubsets[subset] = {}
            }
            if (param === 'NAME') {
                globalSubsets[subset][param] = val.replace(/[\\$\{\\}]/gm, '');
            } else { // all others are integers
                globalSubsets[subset][param] = parseInt(val);
            }
            ...
            let rgx = /subset2combobox \{([A-Z0-9_\s]{1,})\}/gm
            if (match = rgx.exec(line)) {
                const setList = []
                const subsets = match[1].split(' ');
                subsets.forEach(set => {
                    let aSet = globalSubsets[set]
                    if (aSet) {
                        Object.keys(aSet).forEach((p) => {
                            if (obj[receiver][sender][prn].params[p]) {
                                delete obj[receiver][sender][prn].params[p].readonly;
                            }
                        })
                        ...
                        console.log('++++++++++++++++++++++++++++  Set %s not found in %s line %s', set, filename, line)
                    }
                });
                if (!obj[receiver][sender][prn].options[pref]) {
                    obj[receiver][sender][prn].options[pref] = {};
                }
                obj[receiver][sender][prn].options[pref].subsets = setList
            }
        }
    });
